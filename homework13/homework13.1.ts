...
export function DeprecatedMethods ({deprecationReason, replacementMethodName }: 
...
export function Email<Type>(originalMethod: (val: string) => void, context: ClassSetterDecoratorContext) {
        return function (this: Type, value: string) {
...
export function DecoratorArray(decorators: ((value: string) => void)[]) {
...
export function MinLengthModif(minLength: number) {
...
export function MaxLengthModif(maxLength: number) {
        return function (value: string) {
            if (value.toString().length > maxLength){
                throw new Error(`Value must be not greater than 123`)
            }
...
export function EmailModif(value: string) {
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
